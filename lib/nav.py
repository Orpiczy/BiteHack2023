...
class Navigator:
    ...
    def print_status(self, verbose=False, debug_run = False):
        ...
        if verbose:
            print("Visited nodes : ", [node for node, status in self.visited_nodes.items() if status])
            print("NOT Visited nodes : ", [node for node, status in self.visited_nodes.items() if not status])
            print("Visited edges : ", [edge for edge, status in self.visited_edges.items() if status])
            print("NOT Visited edges : ", [edge for edge, status in self.visited_edges.items() if not status])
